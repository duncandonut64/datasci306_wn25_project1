...
```{r}
# Convert the first column (assumed to be date) to Date format
...
```{r}
# Identify columns that start with "consumption_"
consumption_cols <- grep("^consumption_", names(daily), value = TRUE)

# Calculate percentage change for each consumption variable
...
if("holiday" %in% names(daily)) {
  ...
  cat("The dataset does not contain a 'holiday' column.\n")
}
```
...
# Calculate median consumption_power for each year-month combination
monthly_medians <- daily %>%
  group_by(year, month) %>%
  summarise(median_consumption_power = median(consumption_power, na.rm = TRUE)) %>%
  ungroup()

# Identify the year-month with the lowest median consumption_power for each month
lowest_medians <- monthly_medians %>%
  group_by(month) %>%
  slice_min(median_consumption_power, n = 1, with_ties = FALSE) %>%
  ungroup()

# Print the results
print(lowest_medians)
